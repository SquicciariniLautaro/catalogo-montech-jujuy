...
return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-grow">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <span className="text-[10px] text-green-600 font-bold mt-1 ml-1 h-3">
                    {form.precio_usd ? `ARS $ ${(form.precio_usd * cotizacion).toLocaleString('es-AR')}` : ''}
                </span>
                </div>
            </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* RESUMEN DE INVENTARIO ACTIVO */}
        <div className="bg-gray-900 p-5 rounded-2xl shadow-lg grid grid-cols-2 md:grid-cols-4 gap-4 text-white">
            <div>
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Total Equipos</p>
            <p className="font-black text-2xl">{totalStockEquipos}</p>
            </div>
            <div>
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Costo Invertido</p>
            <p className="font-black text-2xl">${totalStockCosto.toLocaleString('es-AR')}</p>
            </div>
            <div>
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Valor de Venta</p>
            <p className="font-black text-2xl text-blue-400">${totalStockVenta.toLocaleString('es-AR')}</p>
            </div>
            <div>
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Ganancia Esperada</p>
            <p className="font-black text-xl text-green-400 leading-tight">
                ${totalStockGananciaUSD.toLocaleString('es-AR')} <span className="text-[10px] text-gray-400 block">ARS ${totalStockGananciaARS.toLocaleString('es-AR')}</span>
            </p>
            </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                {stock.map((celu) => (
                    ...
                    {editandoId === celu.ids[0] ? (
                        ...
                        <td className="p-2 space-y-1">
                            ...
                            <select name="cantidadAEditar" value={formEdicion.cantidadAEditar} onChange={handleChangeEdicion} className="border p-1 text-xs rounded bg-white text-gray-800">
                                {Array.from({length: celu.cantidad}, (_, i) => i + 1).map(n => <option key={n} value={n}>{n} unidad(es)</option>)}
                            </select>
                            </div>
                        </td>
                        <td className="p-2">
                        ...
                        <td className="p-2 text-right space-x-1">
                            <button onClick={() => guardarEdicion()} className="bg-blue-600 text-white px-2 py-1 rounded text-xs font-bold shadow-sm hover:bg-blue-700">Guardar</button>
                        ...
                        <td className="p-4 font-semibold text-gray-800">
                            ...
                            <span className="whitespace-nowrap">{celu.modelo} {celu.capacidad}</span> {renderizarCirculoColor(celu.color)}
                            </div>
                        ...
                        <td className="p-4 text-right space-x-1">
                            ...
                            <button onClick={() => confirmarVenta(celu)} className="bg-green-50 text-green-700 px-2 py-1.5 rounded-lg font-bold hover:bg-green-100 transition text-xs">
                            Vendido
                            </button>
                            <button onClick={() => confirmarBorrado(celu)} className="bg-red-50 text-red-500 px-2 py-1.5 rounded-lg font-bold hover:bg-red-100 transition text-xs">
                            Borrar
                            </button>
                        </td>
                        </>
          ...
          {/* BARRA DE FILTRO POR MES PARA VENTAS */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mt-6 gap-3">
            <h2 className="text-xl font-bold text-gray-800">
            {mesSeleccionado === 'todos' ? 'Historial General de Ventas' : `Ventas de ${formatearNombreMes(mesSeleccionado)}`}
            </h2>
            {mesesDisponibles.length > 0 && (
            <select 
                value={mesSeleccionado} 
                onChange={(e) => setMesSeleccionado(e.target.value)}
                className="border border-gray-300 rounded-lg px-4 py-2 text-sm font-bold bg-white text-gray-700 outline-none shadow-sm cursor-pointer hover:bg-gray-50 transition w-full md:w-auto"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <p className="text-3xl font-black mt-1 text-emerald-600">${gananciaVentasARS.toLocaleString('es-AR')}</p>
            </div>
        </div>
